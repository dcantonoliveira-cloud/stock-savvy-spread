...
import {
  ...
  ArrowDownCircle, ArrowUpCircle, ArrowRight, ChevronRight,
} from 'lucide-react';
import {
      ...
      <div className="grid grid-cols-6 gap-px bg-border rounded-2xl overflow-hidden border border-border">
        {[
          ...
          <button key={k.label} onClick={() => k.path && navigate(k.path)}
            ...
            <p className={`text-xl font-bold tabular-nums ${k.color} ${loading ? 'opacity-30' : ''}`}>{fmtBRL(k.value)}</p>
              ...
              <div className="space-y-1.5 mt-2">
                {expensesByCategory.map((c, i) => (
                  <div key={i} className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-[11px] font-semibold tabular-nums">{fmtBRL(c.value)}</span>
                  </div>
                ))}
              </div>
            </>
            ...
            <div className="divide-y divide-border/50">
              {accounts.map(a => (
                <div key={a.id} className="px-4 py-3 flex items-center gap-3">
                  ...
                  <p className={`text-sm font-bold tabular-nums shrink-0 ${a.balance >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>{fmtBRL(a.balance)}</p>
                </div>
              ))}
              <div className="px-4 py-2.5 flex justify-between items-center bg-muted/30">
                ...
                <span className={`text-sm font-bold tabular-nums ${saldoContas >= 0 ? 'text-foreground' : 'text-red-500'}`}>{fmtBRL(saldoContas)}</span>
              </div>
            </div>
          )}
        </div>
              ...
              {billsReceivable.map(b => (
                <div key={b.id} className="px-4 py-2.5 grid grid-cols-[80px_1fr_90px] gap-2 items-center hover:bg-slate-50 transition-colors group">
                  ...
                  <div className="flex items-center justify-end gap-1">
                    <span className="text-xs font-semibold tabular-nums text-emerald-600">+{fmtBRL(b.value)}</span>
              ...
              {billsPayable.map(b => {
                ...
                return (
                  <div key={b.id} className={`px-4 py-2.5 grid grid-cols-[80px_1fr_90px] gap-2 items-center hover:bg-slate-50 transition-colors ${overdue ? 'bg-red-50/30' : ''}`}>
                    ...
                    <span className="text-xs font-semibold tabular-nums text-right text-red-500">−{fmtBRL(b.amount)}</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="divide-y divide-border/50">
              {upcoming.map((u, i) => (
                <div key={i} className="px-4 py-2.5 flex items-center justify-between gap-2">
                  ...
                  <span className={`text-xs font-bold tabular-nums shrink-0 ${u.type === 'in' ? 'text-emerald-600' : 'text-red-500'}`}>
