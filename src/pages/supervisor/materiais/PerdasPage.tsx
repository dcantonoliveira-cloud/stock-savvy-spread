...
import { Loader2, AlertTriangle, TrendingDown, Package, PackageX, ClipboardMinus } from 'lucide-react';
...
export default function PerdasPage() {
  ...
  const [invLosses, setInvLosses] = useState<InventoryLossRow[]>([]);
  ...
  const load = async () => {
    ...
    // ── Inventory count losses ────────────────────────────────────────────────
    const { data: invData } = await supabase
      .from('material_inventory_losses' as any)
      .select('*, material_items(name, unit)')
      .order('created_at', { ascending: false });

    const invResult: InventoryLossRow[] = ((invData || []) as any[]).map(r => ({
      id: r.id,
      material_item_id: r.material_item_id,
      qty_lost: r.qty_lost,
      reason: r.reason || 'Ajuste de inventário',
      notes: r.notes ?? null,
      created_at: r.created_at,
      item_name: r.material_items?.name || '?',
      unit: r.material_items?.unit || '',
    }));
    setInvLosses(invResult);
  ...
  const filteredInv = invLosses.filter(r =>
    !search ||
    r.item_name.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const totalInvLost = invLosses.reduce((s, r) => s + r.qty_lost, 0);
    ...
    <div>
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-0.5">Histórico de itens perdidos ou avariados</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <div className={`rounded-xl border p-4 text-center shadow-sm ${totalLost > 0 ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-200'}`}>
          ...
          <p className="text-sm text-muted-foreground mt-1">Perdidos em eventos</p>
        </div>
        <div className={`rounded-xl border p-4 text-center shadow-sm ${totalDamaged > 0 ? 'bg-amber-50 border-amber-200' : 'bg-gray-50 border-gray-200'}`}>
          ...
          <p className="text-sm text-muted-foreground mt-1">Avariados em eventos</p>
        </div>
        <div className={`rounded-xl border p-4 text-center shadow-sm ${totalInvLost > 0 ? 'bg-orange-50 border-orange-200' : 'bg-gray-50 border-gray-200'}`}>
          <p className={`text-3xl font-bold ${totalInvLost > 0 ? 'text-orange-600' : 'text-muted-foreground'}`}>{totalInvLost}</p>
          <p className="text-sm text-muted-foreground mt-1">Ajustes de inventário</p>
        </div>
      </div>
      ...
      {/* ── Event losses section ── */}
      <div className="mb-8">
        <div className="flex items-center gap-2 mb-3">
          <TrendingDown className="w-4 h-4 text-red-500" />
          <h2 className="text-sm font-semibold text-foreground uppercase tracking-wider">Perdas em Eventos</h2>
        </div>

        {filtered.length === 0 ? (
          <div className="text-center py-10 text-muted-foreground bg-white rounded-xl border border-border">
            {rows.length === 0 ? (
              <>
      ...
      {/* ── Inventory count losses section ── */}
      <div>
        <div className="flex items-center gap-2 mb-3">
          <ClipboardMinus className="w-4 h-4 text-orange-500" />
          <h2 className="text-sm font-semibold text-foreground uppercase tracking-wider">Ajustes de Inventário</h2>
          <span className="text-xs text-muted-foreground ml-1">(total_qty reduzido manualmente)</span>
        </div>
        ...
        {filteredInv.length === 0 ? (
          <div className="text-center py-10 text-muted-foreground bg-white rounded-xl border border-border">
            ...
            {invLosses.length === 0 && (
              <p className="text-sm mt-1">Ao reduzir a quantidade total de um item no inventário, a diferença aparece aqui</p>
            )}
          </div>
        ) : (
          <div className="bg-white rounded-xl border border-border shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border" style={{ background: 'hsl(40 30% 97%)' }}>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider whitespace-nowrap">DATA</th>
                    <th className="text-left px-3 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider w-full">ITEM</th>
                    <th className="text-left px-3 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">MOTIVO</th>
                    <th className="text-center px-3 py-3 text-xs font-semibold text-orange-500 uppercase tracking-wider">PERDIDO</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-border/50">
                  {filteredInv.map(row => (
                    <tr key={row.id} className="bg-orange-50/10">
                      <td className="px-4 py-3 text-xs text-muted-foreground whitespace-nowrap">
                      ...
                      <td className="px-3 py-3 font-medium text-foreground">{row.item_name}</td>
                      <td className="px-3 py-3 text-muted-foreground text-xs">{row.reason}</td>
                      <td className="px-3 py-3 text-center">
                        <span className="inline-flex items-center gap-1 text-orange-600 font-bold">
                          <TrendingDown className="w-3 h-3" />{row.qty_lost}
                          <span className="font-normal text-xs text-muted-foreground ml-0.5">{row.unit}</span>
                        </span>
                      </td>
                    </tr>
                  ))}
            ...
            <div className="px-4 py-2 border-t border-border bg-muted/10 text-xs text-muted-foreground">
              {filteredInv.length} ajuste{filteredInv.length !== 1 ? 's' : ''} registrado{filteredInv.length !== 1 ? 's' : ''}
            </div>
          </div>
        )}
      </div>
