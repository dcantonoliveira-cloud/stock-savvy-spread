...
type Row = {
  itemId: string; name: string; unit: string; needed: number; inStock: number; toBuy: number;
...
export default function ShoppingListStep({ menuId, onBack }: { menuId: string; onBack: () => void }) {
  ...
  const [menuTitle, setMenuTitle] = useState('Lista de Compras');
  ...
  useEffect(() => {
    const build = async () => {
      ...
      const { data: menuData } = await (supabase.from('event_menus') as any)
        .select('name, events:event_id(event_name)').eq('id', menuId).single();
      if (menuData) setMenuTitle(menuData.events?.event_name || menuData.name || 'Lista de Compras');
      ...
      const [stockRes, subcatsRes, respRes, suppliersRes, profilesRes, groupsRes] = await Promise.all([
        itemIds.length ? (supabase.from('stock_items') as any).select('id, name, unit, current_stock, category, subcategory_id').in('id', itemIds) : Promise.resolve({ data: [] }),
      ...
      const stockById: Record<string, { name: string; unit: string; current_stock: number; category: string; subcategory_id: string | null }> = {};
  ...
  const filteredRows = rows.filter(r => {
    ...
    if (onlyToBuy && r.toBuy <= 0) return false;
    return true;
  });
  ...
  const handlePrint = () => {
    ...
    const rowsHtml = filteredRows.map(r => `<tr>
      ...
      <td class="right ${r.toBuy > 0 ? 'buy' : ''}">${r.toBuy > 0 ? fmtNum(r.toBuy) + ' ' + r.unit : 'OK'}</td>
    </tr>`).join('');
    win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${menuTitle}</title>
      <style>
      ...
      <h2>Lista de Compras</h2><p>${menuTitle} · ${filteredRows.length} itens</p>
  ...
  const handleGeneratePdf = () => {
    ...
    try {
      ...
      pdf.text(`${menuTitle} · ${filteredRows.length} itens`, marginX, y);
      pdf.setTextColor(0);
      y += 24;
      ...
      filteredRows.forEach(r => {
        ...
        if (r.toBuy > 0) {
          ...
          pdf.setFont('helvetica', 'bold');
          pdf.text(`${fmtNum(r.toBuy)} ${r.unit}`, colX[4], y);
          pdf.setFont('helvetica', 'normal');
          pdf.setTextColor(0);
        } else {
      ...
      pdf.save(`Lista de Compras - ${menuTitle}.pdf`);
    } finally {
      setExporting(null);
    }
  };
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      ...
      try {
        ...
        a.href = url; a.download = `Lista de Compras - ${menuTitle}.png`; a.click();
