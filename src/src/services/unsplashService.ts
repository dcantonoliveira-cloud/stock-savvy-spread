...
export async function fetchItemImage(itemName: string): Promise<string | null> {
  ...
  try {
    // Traduz termos comuns para inglês para melhorar resultados
    ...
    if (data.results && data.results.length > 0) {
      // Usa thumb para performance (200x200)
...
// Dicionário básico de tradução para ingredientes comuns em buffet
function translateToEnglish(term: string): string {
  ...
  if (translations[lower]) {
    return translations[lower] + ' food ingredient';
  }

  // Busca tradução parcial
  for (const [pt, en] of Object.entries(translations)) {
    if (lower.includes(pt)) {
      return en + ' food ingredient';
    }
  }

  // Sem tradução: usa o termo original + "food"
