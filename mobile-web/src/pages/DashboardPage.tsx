  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="font-bold text-gray-900 truncate">{eventDisplayName(e)}</p>
        {local ? (
          ...
          e.guest_count != null && (
            <p className="text-xs text-gray-400 mt-0.5">{e.guest_count} convidados</p>
          )
        )}
      </div>
...
export default function DashboardPage() {
  ...
  const degustacoesMes = tastings.filter((d) => {
    ...
    <div className="pb-36 max-w-lg mx-auto">
      ...
      <div className="px-4 space-y-4 pt-4">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-3 gap-3">
            <KpiCard label="Eventos"  value={eventosMes}      sub={mesLabel} accent color="ron" />
            <KpiCard label="Degust."  value={degustacoesMes}  sub={mesLabel} accent color="gold" />
            <KpiCard label="Orçam."   value={orcamentosAbertos} sub="em aberto" accent color="violet" />
          </div>
        )}
          ...
          >
            ...
            <div className="relative">
              ...
              <p className="text-white font-black text-xl leading-tight">
                {eventDisplayName(highlightEvent)}
              </p>
              {eventLocationName(highlightEvent) && (
              ...
              <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/10">
                <p className="text-white/50 text-sm font-semibold">
                  {fmtDate(highlightEvent.event_date ?? undefined)}
                  {highlightEvent.guest_count != null && ` · ${highlightEvent.guest_count} conv.`}
                </p>
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {displayEvents.map((e) => <EventRow key={e.id} e={e} />)}
            </div>
          )}
        </div>
      </div>
